...
La fiscalité de l'or et de l'argent en France repose sur quelques règles claires qu'il faut absolument maîtriser \textbf{avant} de stacker. Le choix du bon régime fiscal à la revente peut vous faire \textbf{économiser plusieurs milliers d'euros}. Ce chapitre vous donne tout ce qu'il faut savoir, avec exemples chiffrés à l'appui.

\subsection{7.1 À l'achat~: la TVA}
...
Métal & TVA \\
\midrule
Or d'investissement & \textbf{0\%} (exonéré) \\
Argent --- pièces cours légal actuel & \textbf{0\%} \\
Argent --- pièces démonétisées (Hercule, Semeuse) & 20\% \\
Argent --- rounds privés, lingots argent classiques & 20\% \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Base légale~:} l'or d'investissement est exonéré de TVA en vertu de l'\textbf{article 298 sexdecies A du Code Général des Impôts}, conformément à la directive européenne 2006/112/CE. Les conditions à remplir pour qu'une pièce soit qualifiée d'or d'investissement~:

\begin{itemize}
  \item Pureté supérieure ou égale à 900\,\permil
  \item Frappée après 1800
  \item Avoir, ou avoir eu, cours légal dans le pays d'émission
  \item Prime ne dépassant pas 80\% de la valeur du métal
\end{itemize}

\textbf{Règle simple côté argent~:} si la pièce est aujourd'hui à cours légal dans son pays d'origine (Maple Leaf canadienne, Eagle américaine, Philharmonique autrichienne, Britannia anglaise, etc.), pas de TVA à l'achat. Si elle est démonétisée (pièces françaises Hercule, Semeuse, Turin) ou n'a jamais eu de cours légal (rounds, lingotins privés), TVA à 20\%.

\subsection{7.2 À la revente~: deux régimes au choix}

Le vendeur particulier doit choisir entre deux régimes fiscaux~: la \textbf{TMP} (Taxe sur les Métaux Précieux) ou la \textbf{TPV} (Taxe sur les Plus-Values).

\subsubsection*{Régime 1 — TMP — Taxe forfaitaire à 11,5\%}

\begin{itemize}
  \item \textbf{Taux global~: 11,5\%} (11\% de taxe + 0,5\% de CRDS)
  \item Calculée sur le \textbf{prix de cession brut}, sans tenir compte du prix d'achat
  \item Applicable même en cas de moins-value
  \item \textbf{Aucun justificatif d'achat requis} --- c'est le régime par défaut
  \item \textbf{Aucun abattement} dans le temps
  \item Déclaration via le formulaire \textbf{2091-SD}, gérée directement par le professionnel acheteur
\end{itemize}

\subsubsection*{Régime 2 — TPV — Taxe sur les plus-values à 36,2\%}

\begin{itemize}
  \item \textbf{Taux global~: 36,2\%} (19\% d'impôt sur le revenu + 17,2\% de prélèvements sociaux)
  \item Calculée uniquement sur la \textbf{plus-value réalisée} --- la différence entre le prix de vente et le prix d'achat
  \item \textbf{Aucun impôt si vous revendez à perte}
  \item \textbf{Justificatif probant obligatoire}~: facture nominative, acte notarié ou attestation de succession
  \item \textbf{Abattement de 5\% par an} dès la 3\textsuperscript{e} année de détention
  \item \textbf{Exonération totale après 22 ans} de détention
  \item Déclaration via le formulaire \textbf{2092-SD}, à déposer dans le mois suivant la vente
\end{itemize}

\subsection{7.3 Tableau d'abattement de la TPV}

L'abattement progressif est ce qui fait toute la puissance du régime TPV pour les stackers de long terme~:

\begin{center}
\begin{tabular}{ccc}
\toprule
\textbf{Années de détention} & \textbf{Abattement} & \textbf{Taux effectif} \\
\midrule
0 à 2 ans & 0\% & 36,2\% \\
5 ans & 15\% & 30,8\% \\
10 ans & 40\% & 21,7\% \\
15 ans & 65\% & 12,7\% \\
20 ans & 90\% & 3,6\% \\
\textbf{22 ans et +} & \textbf{100\%} & \textbf{0\% --- exonération totale} \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Le seuil clé~: 22 ans.} Toute pièce d'or ou d'argent conservée plus de 22 ans avec son justificatif d'achat probant est \textbf{totalement exonérée de plus-value} à la revente. C'est rare en France pour un actif tangible.

\subsection{7.4 Comparatif TMP vs TPV}

\begin{center}
\begin{tabular}{lll}
\toprule
\textbf{Critère} & \textbf{TMP} & \textbf{TPV} \\
\midrule
Assiette de calcul & Prix total de vente & Plus-value uniquement \\
Taux & 11,5\% & 36,2\% (avant abattement) \\
Justificatif d'achat & Non requis & Obligatoire (probant) \\
Abattement temporel & Aucun & 5\%/an dès 3\textsuperscript{e} année \\
Moins-value & Taxe due quand même & Aucun impôt \\
Exonération possible & Jamais & Après 22 ans \\
Formulaire & 2091-SD (géré pro) & 2092-SD (à déposer 1 mois) \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Quel régime choisir~?} La règle générale~:
\begin{itemize}
  \item \textbf{Plus-value faible ou nulle~:} la TMP (11,5\% du brut) est souvent plus lourde que la TPV (36,2\% d'une petite plus-value)
  \item \textbf{Plus-value forte~:} comparer les deux selon votre durée de détention
  \item \textbf{Détention longue (>10 ans)~:} la TPV avec abattement devient très avantageuse
  \item \textbf{Moins-value~:} la TPV est imbattable (aucun impôt vs 11,5\% en TMP)
  \item \textbf{Pas de factures~:} la TMP s'impose par défaut, vous n'avez pas le choix
\end{itemize}

\newpage

\subsection{7.5 Exemples chiffrés}

\subsubsection*{Exemple 1 --- Or (Krugerrand 1 oz)}

\textbf{Scénario~:} vente d'\textbf{une} pièce Krugerrand 1 oz or à 4\,465\,€.

\begin{center}
\begin{tabular}{lrr}
\toprule
\textbf{Régime} & \textbf{Calcul} & \textbf{Taxe due} \\
\midrule
TMP (sans facture) & 4\,465\,€ $\times$ 11,5\% & \textbf{513,48\,€} \\
TPV (achetée 3\,200\,€ il y a 5 ans) & (4\,465 -- 3\,200) $\times$ 36,2\% $\times$ (1--15\%) & 388,90\,€ \\
TPV (achetée 3\,200\,€ il y a 22 ans) & 0\,€ --- exonération totale & \textbf{0\,€} \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Verdict~:} avec une plus-value modérée et 5 ans de détention, la TPV reste plus avantageuse de \textbf{124\,€}. Après 22 ans, l'avantage devient absolu.

\subsubsection*{Exemple 2 --- Argent (Maple Leaf 1 oz)}

\textbf{Scénario~:} vente de \textbf{10} Maple Leaf argent 1 oz, achetées 300\,€ en 2023, revendues 850\,€ en 2028 (\textbf{5 ans} de détention).

\begin{center}
\begin{tabular}{lrr}
\toprule
\textbf{Régime} & \textbf{Calcul} & \textbf{Net en poche} \\
\midrule
TMP & 850\,€ $\times$ 11,5\% = 97,75\,€ de taxe & \textbf{752,25\,€} \\
TPV (5 ans, abattement 15\%) & (850 -- 300) $\times$ 36,2\% $\times$ (1--15\%) = 169,24\,€ & 680,76\,€ \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Verdict~:} ici la TMP vous laisse \textbf{71,49\,€ de plus} dans la poche. Pour les petites plus-values comme c'est souvent le cas avec l'argent, la TMP est plus avantageuse que la TPV \textit{avant} le seuil critique de détention longue.

\subsubsection*{Le piège classique de l'argent}

L'argent étant beaucoup plus accessible (1 oz $\approx$ 30\,€ vs 1 oz d'or à 4\,500\,€), beaucoup de stackers en accumulent de grosses quantités. \textbf{Attention au seuil de 5\,000\,€} par opération~:

\begin{itemize}
  \item Sous 5\,000\,€~: aucune déclaration automatique, marché secondaire entre particuliers discret
  \item Au-dessus de 5\,000\,€ via un professionnel~: déclaration automatique aux impôts
\end{itemize}

\subsection{7.6 Donations en pièces d'or et d'argent}

Les pièces physiques se transmettent comme n'importe quel autre bien~: vous bénéficiez des \textbf{abattements de donation} classiques, \textbf{renouvelables tous les 15 ans}.

\begin{center}
\begin{tabular}{lr}
\toprule
\textbf{Lien de parenté} & \textbf{Abattement par donation} \\
\midrule
...
Personnes handicapées (supplément) & + 159\,325\,€ \\
\midrule
Concubin ou personne non parentée & 0\,€ --- taxation 60\% \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Stratégie patrimoniale~:} un couple peut transmettre jusqu'à \textbf{200\,000\,€ par enfant} en pièces d'or ou d'argent \textbf{tous les 15 ans}, sans aucun droit de donation. Sur 30 ans, c'est \textbf{400\,000\,€ par enfant} totalement exonéré.

Au-delà de ces abattements, le barème classique des droits de donation s'applique selon le lien de parenté (5\% à 45\% pour les enfants, jusqu'à 60\% pour les non-parents).

\subsection{7.7 Conseils stratégiques du stacker}

\textbf{Conseil n°1~:} \textbf{conservez systématiquement vos factures} d'achat avec date, prix et identification précise de la pièce. Sans elles, la TMP s'impose par défaut, même si la TPV serait bien plus avantageuse.

\textbf{Conseil n°2~:} \textbf{privilégiez une détention longue}. Après 22 ans, vos pièces sont totalement exonérées sous le régime TPV. C'est rare en France pour un actif tangible.

\textbf{Conseil n°3~:} \textbf{utilisez les donations planifiées}. Tous les 15 ans, vous pouvez transmettre 100\,000\,€ par enfant et par parent en franchise totale. Cumulé sur une vie d'investisseur, c'est un transfert de patrimoine massif sans fiscalité.

\textbf{Conseil n°4~:} \textbf{comparez les primes avant d'acheter}. Une prime de 10\% au lieu de 5\% impacte directement votre plus-value future et donc votre fiscalité TPV. Un comparateur indépendant comme \textbf{BullionRadar} affiche les prix de plusieurs dealers en temps réel.

\textbf{Conseil n°5~:} pour l'argent, \textbf{privilégiez les pièces à cours légal} (Maple Leaf, Britannia, Philharmonique, Eagle, etc.) afin d'éviter la TVA à 20\% qui s'applique aux rounds privés et aux pièces démonétisées.

\subsection{7.8 Déclaration et seuil de 5\,000\,€}

Les transactions de plus de \textbf{5\,000\,€ via un professionnel} font l'objet d'une déclaration automatique aux impôts par le vendeur, qui prélève la TMP à la source (sauf option pour la TPV par dépôt du formulaire 2092-SD).

En dessous de 5\,000\,€, le marché secondaire entre particuliers reste discret, mais reste soumis aux mêmes règles fiscales. Vous devez déclarer vos plus-values vous-même si vous optez pour la TPV.

\textbf{Conservez tous vos justificatifs} (factures, attestations, courriers) pendant toute la durée de détention~: les contrôles fiscaux peuvent intervenir jusqu'à 3 ans après la vente.

\subsection{7.9 FAQ fiscalité --- questions fréquentes du stacker}

\textbf{Q1 --- Puis-je acheter mes pièces en \textit{cash} pour rester anonyme~?}

Oui, en dessous de \textbf{1\,000\,€} par transaction (limite fixée par l'article L112-6 du Code monétaire). Au-delà, le règlement par chèque, virement ou CB est obligatoire, et le dealer demande votre identité. C'est la loi française anti-blanchiment, pas une règle BullionRadar.

\textbf{Q2 --- Si j'achète à l'étranger (Suisse, Belgique), quelle fiscalité~?}

La fiscalité est celle de votre \textbf{pays de résidence fiscale} au moment de la revente. Un résident français qui revend en Suisse paie la fiscalité française (TMP ou TPV). En revanche, l'achat est libre de droits de douane à l'intérieur de l'UE pour l'or d'investissement (exonéré de TVA dans tous les pays UE).

\textbf{Q3 --- Que se passe-t-il en cas d'héritage~?}

Les pièces transmises par succession entrent dans l'actif successoral et sont soumises aux droits de succession selon le barème classique. L'héritier reçoit \textbf{le prix de marché au jour du décès} comme prix d'acquisition pour le calcul ultérieur des plus-values. Bonne nouvelle~: ce \textit{step-up} efface toute la plus-value accumulée du vivant.

\textbf{Q4 --- Et si je vends entre particuliers~?}

La vente entre particuliers reste soumise à la même fiscalité, mais sans déclaration automatique. Vous devez \textbf{déclarer vous-même} via les formulaires 2091-SD ou 2092-SD dans le mois suivant la vente. Pas de zone grise~: l'administration peut contrôler.

\textbf{Q5 --- Les pièces de collection sont-elles fiscalisées différemment~?}

Oui. Les pièces dont la prime dépasse \textbf{80\% de la valeur du métal} ou qui sont uniquement vendues pour leur intérêt numismatique relèvent du régime des \textbf{objets précieux} (TVA à l'achat, TPV à 36,2\% à la revente, abattement après 12 ans).

\textbf{Q6 --- Faut-il déclarer son stack dans l'IFI ou l'ISF~?}

Non. Depuis 2018, l'\textbf{IFI} (Impôt sur la Fortune Immobilière) ne concerne que l'immobilier. Les pièces d'or et d'argent ne sont \textbf{plus dans aucune assiette d'imposition sur la fortune}. C'est un avantage majeur de l'or physique par rapport aux SCPI ou à l'immobilier locatif.

\textbf{Q7 --- Et la TVA sur marge pour les pièces d'argent démonétisées~?}

Les dealers professionnels peuvent appliquer le régime de la \textbf{TVA sur marge} pour les pièces d'argent démonétisées achetées d'occasion~: la TVA ne s'applique alors que sur leur marge brute (~5--15\%), pas sur le prix total. Pour le particulier, cela rend les Hercule et Semeuse argent beaucoup plus abordables qu'avec une TVA à 20\% sur la totalité.

\textbf{Q8 --- Que vérifier sur une facture pour qu'elle soit valable en TPV~?}

Une facture \textbf{probante} doit comporter~: votre nom et prénom complets, le nom et le SIRET du vendeur, la date d'achat, la désignation précise de chaque pièce (millésime, poids, métal, pureté), le prix unitaire et le total. Un simple ticket de caisse \textit{anonyme} ne suffit pas pour prouver votre titularité.

\textbf{Q9 --- Que faire si je perds mes factures d'achat~?}

Demandez à votre dealer un \textbf{duplicata} de facture, possible si la vente date de moins de 10 ans (durée de conservation comptable obligatoire). Sinon, vous serez \textbf{automatiquement basculé sur la TMP} à 11,5\% du prix total --- ce qui peut représenter une fiscalité bien plus lourde que la TPV avec abattement. C'est pour cela qu'il est crucial de tout numériser dès l'achat et de stocker dans plusieurs lieux (cloud chiffré + disque dur externe + impression papier).

\textbf{Q10 --- L'or hérité avant 2018 et l'IFI/ISF~: y a-t-il un risque rétroactif~?}

Non. Depuis le 1\textsuperscript{er} janvier 2018, les métaux précieux sont \textbf{définitivement sortis} de l'assiette de l'impôt sur la fortune en France. Aucune action rétroactive n'est possible, et même si vous étiez assujetti à l'ISF entre 1989 et 2017, vos transmissions actuelles sont fiscalement indépendantes de cette époque.

\textbf{Q11 --- Et si je transfère mes pièces vers un coffre étranger (Suisse, Singapour)~?}

Le simple fait de \textbf{détenir} des pièces dans un coffre étranger ne change rien à votre fiscalité française si vous êtes résident fiscal en France. En revanche, à partir de \textbf{50\,000\,€ de valeur stockée à l'étranger}, vous devez \textbf{déclarer le compte/coffre étranger} sur le formulaire 3916 avec votre déclaration de revenus annuelle. Omettre cette déclaration peut coûter une amende de 1\,500\,€ par compte non déclaré (10\,000\,€ si pays non coopératif).
...
\textit{Cette section présente un résumé clair des règles fiscales applicables aux pièces d'or et d'argent en France au moment de la rédaction de ce guide (édition 2026). Pour toute décision patrimoniale importante, consultez un fiscaliste ou un conseiller financier agréé. BullionRadar est un comparateur indépendant et ne fournit pas de conseil fiscal personnalisé.}
...
\begin{itemize}
  \item \textit{Le Standard Argent} --- Professeur Antal Fekete (théorie monétaire de l'argent, rôle des métaux précieux dans un système monétaire sain)
  \item \textit{Le Standard Or} --- Professeur Antal Fekete (analyse de l'étalon-or, critique du système monétaire fiat et plaidoyer pour un retour à la monnaie réelle)
\end{itemize}
...
% APPEL FINAL — PARTAGE
% =============================================================================
\section*{\faShareAlt~~Faites connaître BullionRadar}
\addcontentsline{toc}{section}{Faites connaître BullionRadar}

Vous avez trouvé ce guide utile~? \textbf{Soutenez le projet et aidez d'autres stackers à se former~!}

BullionRadar est un projet \textbf{indépendant}, gratuit, sans publicité intrusive. Notre seule récompense~: voir le nombre de stackers français mieux informés grandir chaque mois.

\medskip

\begin{center}
{\Large \faThumbsUp~~\textbf{3 actions qui font la différence}}
\end{center}

\medskip

\begin{itemize}[leftmargin=2em,itemsep=1em]
  \item[\faShare] \textbf{Partagez ce PDF} autour de vous --- famille, collègues, amis intéressés par l'or et l'argent. Forum, groupe WhatsApp, Discord, Telegram\ldots tout est bon~!

  \item[\faYoutube] \textbf{Abonnez-vous à notre chaîne YouTube} et partagez nos vidéos~: analyses de pièces, fiscalité, stratégies de stacking. Chaque abonné et chaque vue compte pour faire grandir la communauté française des stackers.

  \item[\faGlobe] \textbf{Parlez de \href{https://bullionradar.fr}{bullionradar.fr}} sur vos réseaux sociaux~: Facebook (\faFacebook), X / Twitter (\faXTwitter), LinkedIn (\faLinkedin), Reddit. Un simple post peut faire découvrir le comparateur à des dizaines d'investisseurs.
\end{itemize}
...
\faHeart~\textbf{Soutien} &
\faComments~\textbf{Communauté} &
\faRocket~\textbf{Croissance}
\end{tabular}
\end{center}

\medskip

\subsection*{\faLink~~Liens utiles}

\begin{itemize}[leftmargin=2em]
  \item \faGlobe~~\href{https://bullionradar.fr}{\textbf{bullionradar.fr}} --- Comparateur de prix en temps réel chez 3 dealers français
  \item \faFileAlt~~\href{https://bullionradar.fr/fiscalite}{bullionradar.fr/fiscalite} --- Guide complet fiscalité avec tableaux interactifs
  \item \faYoutube~~\href{https://www.youtube.com/@BullionRadar}{Chaîne YouTube BullionRadar} --- Analyses pièces, fiscalité, stratégies
  \item \faEnvelope~~\href{mailto:contact@bullionradar.fr}{contact@bullionradar.fr} --- Suggestions, partenariats, retours
\end{itemize}
...
\medskip
\centering
{\Large\textbf{\textcolor{gold}{Merci de votre confiance~!}}}
\medskip

\textit{Plus on est nombreux à stacker, plus la communauté française\\
des investisseurs en métaux précieux devient forte.}

\medskip

{\large\faStar~\faStar~\faStar~\faStar~\faStar}
...
\begin{center}
\textit{Édition 2026 --- BullionRadar.fr --- Le Petit Guide du Stacker}
\end{center}
